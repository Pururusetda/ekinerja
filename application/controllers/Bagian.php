...
class Bagian extends CI_Controller {	
	...
	{		
		...
		$data['allPengguna'] = $this->M_Bagian->getAll();
		$this->load->view("layout/header",$data);
		$this->load->view("bagian/show");
	...
	public function post_create() {
        if($this->session->userdata('level') != 'Administrator') redirect(site_url());
		if($this->input->post() == FALSE) redirect("pegawai/create");
		...
		if($this->M_Pegawai->create($username,$password,$email,$name,$level) == TRUE) {
	...
	public function edit($id) {
        if($this->session->userdata('level') != 'Administrator') redirect(site_url());
		$data['user']	= $this->M_Pegawai->getDetail($id);
	...
	public function post_delete() {
		...
		if($this->M_Pegawai->delete($id,$username) == TRUE) {
			$this->session->set_flashdata('message', 'Success');
			redirect("user");
		} else {			
			$this->session->set_flashdata('message', 'Gagal');
